package com.archimatetool.export.svg;

import org.eclipse.osgi.util.NLS;

public class Messages extends NLS {

    private static final String BUNDLE_NAME = "com.archimatetool.export.svg.messages"; //$NON-NLS-1$

    public static String PDFExportProvider_0;

    public static String PDFExportProvider_1;

    public static String PDFExportProvider_2;

    public static String SVGExportProvider_0;

    public static String SVGExportProvider_1;

    public static String SVGExportProvider_2;

    public static String SVGExportProvider_3;

    public static String SVGExportProvider_4;

    public static String SVGExportProvider_5;

    public static String SVGExportProvider_6;

    public static String SVGExportProvider_7;

    public static String SVGExportProvider_8;

    static {
        // initialize resource bundle
        NLS.initializeMessages(BUNDLE_NAME, Messages.class);
    }

    private Messages() {
    }
}
